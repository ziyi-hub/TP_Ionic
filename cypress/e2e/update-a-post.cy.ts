...
describe('Update a post', () => {
  it('passes', () => {
    ...
    cy.get('ion-item-sliding[data-testid="posts"]')
      ...
      .click({force:true});
    cy.get('ion-input[data-testid="post-name"]').first().type("{selectall}Post B")
    cy.get('ion-textarea[data-testid="post-desc"]').type('{selectall}Lorem Ipsum is simply dummy text of the printing and typesetting industry.')
    cy.get('[data-testid="confirm-new-post-modal-btn"]').click()
    cy.get('[data-testid="posts"]').should('have.length', 1)
    cy.get('[data-testid="posts"]').find('ion-label').contains('Post B')
